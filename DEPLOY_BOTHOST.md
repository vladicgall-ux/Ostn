...
В этом репозитории есть Telegram-бот (`bot.py`), который запускает
...
Есть два варианта — если платформа сама определяет язык и точку входа,
но раз за разом ошибается (например, вместо Python собирает Node.js),
используйте вариант Б, он не зависит от автоопределения.

### Вариант А — режим "Python"

В панели BotHost создайте новый Python-проект, указывающий на этот
репозиторий, и задайте:

- **Точка входа / главный файл:** `bot.py`
- **Файл зависимостей:** `requirements.txt` (уже включает зависимости
  самого SpiderFoot плюс `python-telegram-bot` и `python-dotenv`)
- **Версия Python:** 3.11 (рекомендуется; совместима с зависимостями
  SpiderFoot)

### Вариант Б — режим "Docker" / свой Dockerfile

Если у платформы есть режим сборки по собственному `Dockerfile` —
выберите его. В репозитории уже есть корневой `Dockerfile`, настроенный
на этой ветке так, чтобы по умолчанию запускать именно `bot.py`
(он сам поднимает SpiderFoot внутри контейнера на `127.0.0.1`). Никакой
точки входа указывать не нужно — платформа просто соберёт образ и
запустит его.
...
Запустите/перезапустите проект в панели BotHost. При старте `bot.py`:
